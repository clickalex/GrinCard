...
(function (root, factory) {
  ...
  function isLinkVisible(link, tier, tokenVerdict) {
    ...
    if (link.visibility !== VISIBILITY.FOLLOWERS_ONLY) return true; // public
  ...
  function isPrivate(link) { return link && link.visibility === VISIBILITY.FOLLOWERS_ONLY; }
  ...
  function validateProfile(raw) {
    ...
    p.links.forEach(function (link, i) {
      ...
      if (link.visibility !== VISIBILITY.PUBLIC && link.visibility !== VISIBILITY.FOLLOWERS_ONLY) {
        warnings.push(link.id + ': unknown visibility "' + link.visibility + '", defaulting to public');

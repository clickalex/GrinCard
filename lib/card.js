...
(function (root, factory) {
  ...
  function buildCard(profile, options) {
    ...
    return {
      template: template,
      widthMm: TPL.GEOMETRY.widthMm,
      heightMm: TPL.GEOMETRY.heightMm,
      front: buildFront(profile, template, options),
      back: buildBack(profile, template, options)
    };
  }
  ...
  function toSVG(card, side, options) {
    ...
    var bleed = options.bleed ? TPL.GEOMETRY.bleedMm : 0;

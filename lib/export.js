...
(function (root, factory) {
  ...
  function prepareCard(profile, options) {
    ...
    if (!photo) return Promise.resolve({ card: Card.buildCard(profile, options), warnings: warnings });

    return toEmbeddedDataUrl(photo).then(function (dataUrl) {
      if (!dataUrl) {
        warnings.push('The photo could not be embedded (the host blocked cross-origin reads). ' +
          'Upload the photo instead of linking to it, or it will be left out of the download.');
      }
      return {
        card: Card.buildCard(profile, Object.assign({}, options, { photoDataUrl: dataUrl })),
        warnings: warnings
      };
    }).catch(function (err) {
      warnings.push('Photo skipped: ' + err.message);
  ...
  function toPngCanvas(prepared, side, opts) {
    ...
    var W = prepared.card.widthMm + (opts.bleed ? TPL.GEOMETRY.bleedMm * 2 : 0);
    var H = prepared.card.heightMm + (opts.bleed ? TPL.GEOMETRY.bleedMm * 2 : 0);

...
(function (root, factory) {
   ...
   *
   * `<body data-profile-dir="…">` overrides it — that is how the example gallery
   * and the tests point at `examples/` instead of the owner's own
   * `profile-data/`. Otherwise it is the site root's `profile-data/`, expressed
   * relative to this page so it works from any depth.
   */
  function dataDir() {
    if (typeof document !== 'undefined' && document.body) {
    ...
    return pageRelative(rootRelative('profile-data/'));
  }
      ...
      return Promise.all(names.map(function (name) {
        ...
        return list.filter(Boolean).map(function (p) {
          return {
            ...
            url: profileUrlFor(p.username, p),
            _starter: !!p._starter
          };
        });
      });
